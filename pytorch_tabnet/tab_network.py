...
class TabNetCustomPretraining(torch.nn.Module):
    def __init__(
        self,
        input_dim,
        pretraining_ratio=0.2,
        ...
        super(TabNetCustomPretraining, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            # set prior of encoder with obf_mask
            prior = 1 - obf_vars
            embedded_e, _ = self.encoder(masked_x, prior=prior)
            res = self.decoder(embedded_e)
            return res, embedded_x, obf_vars, embedded_e
        else:
            embedded_e, _ = self.encoder(embedded_x)
            res = self.decoder(embedded_e)
            return res, embedded_x, torch.ones(embedded_x.shape).to(x.device), embedded_e
